...
# Determine a square that can secure a win / avert loss
def crucial_third(board, marker):
    for line in WINNING_LINES:
        markers_in_line = [board[square] for square in line]
        if markers_in_line.count(marker) == 2:
            try:
                return line[markers_in_line.index(' ')]
            except ValueError:
                continue
    return None
...
def computer_medium(board):
    square = crucial_third(board, HUMAN_MARKER)
...
def prepare_match():
    ...
    return difficulty, game_count, scoreboard

def host_match():
    explain_basics()
    difficulty, game_count, scoreboard = prepare_match()
