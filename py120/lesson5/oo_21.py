...
class TwentyOneGame:
    ...
    def deal_cards(self):
        self.player.hand = [self.deck.cards.pop(), self.deck.cards.pop()]
        self.player.update_hand_value()
        self.computer.hand = [self.deck.cards.pop(), self.deck.cards.pop()]
        self.computer.update_hand_value()

    def show_cards(self, final_display=False):
        system('clear')
        for player in (self.computer, self.player):
            rows = ['', '', '', '', '']
    ...
    def dealer_turn(self):
        while self.computer.hand_value < Dealer.HIT_CAP:
            self.computer.hand.append(self.deck.cards.pop())
            self.computer.update_hand_value()

    def display_welcome_message(self):
        system('clear')
        print('Let\'s play Twenty-One!')
        print(f'Both you and dealer get a ${TwentyOneGame.CREDIT_LIMIT} credit.')
        print('Game ends when one of the players has no credit left. Good luck!')
    ...
    def display_round_result(self):
        ...
        if self.player.is_busted():
            ...
            self.update_credits(self.computer, self.player)
        else:
            print(f'Dealer\'s hand value is {self.computer.hand_value}')
            if self.computer.hand_value > TwentyOneGame.MAX_HAND:
                print('Dealer busted, you win!')
                self.update_credits(self.player, self.computer)
            elif self.player.hand_value > self.computer.hand_value:
    ...
    def prepare_round(self):
        self.deck.cards = self.deck.prepare()
        self.player.hand = []
        self.player.hand_value = 0
        self.computer.hand = []
        self.computer.hand_value = 0
